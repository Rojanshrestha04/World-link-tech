  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        ...
        description: `This is a sample description for course ${id}.`,
        instructorBio: `Bio of instructor for course ${id}.`,
        prerequisites: ["Basic English", "Computer Literacy"],
        learningOutcomes: ["Understand X", "Apply Y"],
        courseMaterials: ["Textbook Z", "Online resources"],
        active: true,
        featured: false,
      })
    }, 500)
...
export default function EditCoursePage() {
  ...
  useEffect(() => {
    if (courseId) {
      fetchCourseById(courseId).then((data: any) => {
        setCourseData(data)
        setLoading(false)
      })
    }
